...
import os
import mimetypes
import httpx # For making HTTP requests to ImgBB
from typing import Annotated, Tuple, Optional

# ImgBB API Key provided by the user
IMGBB_API_KEY = "7c765c9cefe9d5e1e8bf91569bf35cb9"

def upload_image_to_imgbb_and_get_markdown(
    image_path: Annotated[str, "Local path to the image file to upload."]
) -> str:
    """
    Uploads an image from the given local path to ImgBB as a binary file
    ...
    Returns:
        A string containing the direct URL to the image if successful,
        or an error message string if the upload fails.
    """
    print(f"DEBUG: Attempting to upload image from path: {image_path} to ImgBB.")

    if not IMGBB_API_KEY:
        error_msg = "ERROR: IMGBB_API_KEY is not set/empty. Cannot upload image."
        print(error_msg)
        return error_msg
    ...
    try:
        ...
        files_payload = {
            'image': (file_name, image_bytes, content_type) 
        }
            
        print(f"DEBUG: Uploading '{file_name}' ({content_type}) to ImgBB as binary...")
        ...
        if response.status_code == 200:
            ...
            if response_data.get("success") and response_data.get("data", {}).get("url"):
                image_url = response_data["data"]["url"]
                if 'display_url' in response_data["data"]: 
                    image_url = response_data["data"]["display_url"]
                
                cleaned_url = image_url.strip()
                print(f"DEBUG: Successfully uploaded to ImgBB. URL: {cleaned_url}")
                return cleaned_url # Return only the URL
            else:
                error_detail = response_data.get("error", {}).get("message", str(response_data))
                error_msg = f"ImgBB upload failed. API Response: {error_detail}"
                print(f"ERROR: {error_msg}")
                return f"Error: {error_msg}"
        else:
            error_msg = f"ImgBB API error. Status: {response.status_code}, Response: {response.text}"
            print(f"ERROR: {error_msg}")
            return f"Error: {error_msg}"

    except FileNotFoundError:
        return f"Error: Image file not found at '{image_path}' (FileNotFoundError during processing)."
    except Exception as e:
        import traceback
        error_msg = f"Failed to upload image '{image_path}' to ImgBB: {type(e).__name__}: {str(e)}\n{traceback.format_exc()}"
        print(f"ERROR: {error_msg}")
        return f"Error: {error_msg}"
